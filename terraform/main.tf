...
resource "aws_lb_listener" "juice_shop_http" {
  load_balancer_arn = aws_lb.main.arn
  port              = "80"
  protocol          = "HTTP"

  default_action {
    type = var.ssl_certificate_arn != null && var.enable_https_redirect ? "redirect" : "forward"

    dynamic "redirect" {
      for_each = var.ssl_certificate_arn != null && var.enable_https_redirect ? [1] : []
      content {
        port        = "443"
        protocol    = "HTTPS"
    ...
    dynamic "forward" {
      ...
      content {
        target_group {
          arn = aws_lb_target_group.juice_shop.arn
        }
      }
    }
  }
...
resource "aws_lb_listener" "juice_shop_https" {
...
resource "aws_ecs_service" "juice_shop" {
  ...
  depends_on = [aws_lb_listener.juice_shop_http]
...
resource "aws_s3_bucket" "waf_logs" {
  ...
  # Ensure bucket contents (including versioned objects) are purged on destroy
  force_destroy = true
...
resource "aws_s3_bucket_lifecycle_configuration" "waf_logs" {
  ...
  rule {
    ...
    # Apply to all objects (AWS provider v5 requires one of prefix or filter)
    filter {
      prefix = ""
    }
...
resource "aws_kinesis_firehose_delivery_stream" "waf_logs" {
  ...
  tags = {
    Name = "${var.project_name}-waf-logs-firehose"
  }
}
...
# WAF Module - test ci trigger - softfail
module "edge_waf" {
  ...
  kinesis_firehose_arn     = var.enable_waf_logging ? aws_kinesis_firehose_delivery_stream.waf_logs[0].arn : null
  allowed_countries        = ["US", "CA", "GB"] # Customize as needed
  rate_limit_per_5_minutes = var.rate_limit_per_5_minutes
